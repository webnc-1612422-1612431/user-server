...
module.exports = {
    ...
    countByTeacherId: teacherid => {
        return db.load(`select count(*) as count, `
            ...
            + `avg(rate) as rate, `
            + `sum(case when contract.state = 2 then 1 else 0 end) as success from contract where teacherid = '${teacherid}'`);
    },

    topTenRate: () => {
        return db.load(`SELECT user.*, `
            + `sum(case when contract.state = 2 then 1 else 0 end) as success, `
